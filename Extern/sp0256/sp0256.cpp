...
/*
   ...
   Adapted for SimCoupe by Simon Owen.
...
sp0256_device::sp0256_device(uint32_t rate) :
    m_rate(rate)
{
    /* -------------------------------------------------------------------- */
    ...
    auto wind = (m_rate > 10000) ? (m_rate / 5000 + 3) : 20000 / m_rate + 1;
    if (wind < 1) wind = 1;
    m_window.resize(wind);
}

void sp0256_device::load_rom(uint16_t addr, const std::vector<uint8_t>& rom)
{
    std::copy(rom.begin(), rom.end(), m_rom.begin() + addr);
}

void sp0256_device::reset()
{
    ...
    // TODO: real device sets m_lrq 50 ticks after reset.
}
...
uint16_t sp0256_device::spb640_r(uint16_t offset)
{
    /* -------------------------------------------------------------------- */
...
void sp0256_device::spb640_w(uint16_t offset, uint16_t data)
{
    if (offset == 0)
    {
        ald_w(data & 0xff);
...
void sp0256_device::sound_stream_update(stream_sample_t* output, int samples)
{
    constexpr bool pal_mode = true;
    constexpr auto sys_clock = pal_mode ? 4'000'000 : 3'579'545;
    constexpr auto clock_per_samp = pal_mode ? 400 : 358;
    ...
    while (output_index < samples)
    {
        ...
        while (m_sc_tail != m_sc_head)
        {
            ...
            /* -------------------------------------------------------------*/
            /* Window resampling from original ivoice.c source code.        */
            /* -------------------------------------------------------------*/

            ws = s = m_scratch[m_sc_tail++];
            m_sc_tail &= SCBUF_MASK;

            m_sample_frc += m_rate * clock_per_samp;

            while (m_sample_frc > sys_clock)
            {
                m_sample_frc -= sys_clock;

                if (m_rate >= 10000)
                {
                    m_wind_sum -= m_window[m_wind_ptr];
                    m_window[m_wind_ptr++] = s;
                    if (m_wind_ptr >= m_window.size())
                        m_wind_ptr = 0;

                    m_wind_sum += s;
                    ws = m_wind_sum / static_cast<int32_t>(m_window.size());
                }

                output[output_index++] = ws;
        ...
        if (length > 0) do
        {
            ...
            {
                did_samp += m_filt.update(do_samp, m_scratch.data(), &m_sc_head);
            }
