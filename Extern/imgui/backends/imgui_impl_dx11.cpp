...
//  2022-03-22: DirectX11: Changed to use pre-compiled shaders and sRGB [SimCoupe].
    ...
    // Create the vertex shader
    {
        ...
        static const char* vertexShader =
            ...
            {\
              ...
              output.col.xyz = pow(input.col.xyz, 2.2f);\
...
#else
        if (bd->pd3dDevice->CreateVertexShader(g_ImGui_VS, sizeof(g_ImGui_VS), NULL, &bd->pVertexShader) != S_OK)
#endif
        {
...
#else
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_ImGui_VS, sizeof(g_ImGui_VS), &bd->pInputLayout) != S_OK)
#endif
        {
            //vertexShaderBlob->Release();
            return false;
...
#else
        if (bd->pd3dDevice->CreatePixelShader(g_ImGui_PS, sizeof(g_ImGui_PS), NULL, &bd->pPixelShader) != S_OK)
#endif
        {
